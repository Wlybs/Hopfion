...
- **2026-06-15 首算 `delta=1 vs delta=2` 有限差分收敛门 FAIL**（G_XY ~80%，D 对角 ~19%）→ 当时零 G 结果尚不可发布。根因判定为单胞 `np.roll` 差分过粗（纹理管子仅 ~6–7 胞宽）。
- **2026-07-03 TC.1 收敛阶梯已重算，GATE GC1 PASS**（`results_thiele_GD_convergence_20260703/`）：Richardson(δ=1..4 Neville 外推) + 4 阶 5 点 stencil + 原生 100³ 谱导数 + 2×/4× Fourier 超采样谱导数，五法两两相对自然尺度 S 的最大分歧仅 0.19%（G）/ 0.0003%（D），远低于 5% 门槛；张量代数门（反对称/对称）与 skyrmion 独立标定（非同义反复，用固定拓扑荷目标 Q=-1 校验）均 PASS。闭环平动 G_XY/G_XZ/G_YZ 用最细方法（4×超采样谱导数）仍全部低于 3% 噪声底，**零 G 收敛现已确认**（不再是"未确认"）。**尚不发布 Hall 角**：仍需 TC.4 实测偏转角，在 GC-FINAL 处裁决；TC.3（skyrmion 匹配分辨率误差棒）仍未跑，是"已确认"标签在证据表（§9）里保留条件的原因。bd: Hopfion-960（closed）。
...
- **TC.1（零仿真）收敛阶梯**（嫁接自理论方案）：Richardson δ 阶梯（δ=1,2,3,4）+ 4 阶差分 stencil + 谱(FFT)导数 + 2×/4× 超采样。所有小 G 变化相对自然尺度 S 表达，要求零判定在每个 δ 都成立。**✅ 2026-07-03 完成**，见 `07_thiele_theory_model/results_thiele_GD_convergence_20260703/`（脚本自含：每次运行自解压 `m000020.ovf`）。
  - **GATE GC1（全论文枢纽）**：{4 阶 FD, Richardson, 谱导数, 2×/4×超采样} 是否互相 5% 内一致？**✅ PASS**（最大两两分歧 G 0.19%、D 0.0003%，均相对自然尺度 S）→ 收敛后的 G/D 已释放；零 G 判定在闭环三分量（用最细方法 4×超采样谱导数）均确认。不再需要 TC.2。
...
| # | 主张 | 等级 | 措辞/依赖 |
|---|---|---|---|
| 1 | 平动 G/D 3×3 块数值 | TC.1 收敛门已 PASS（2026-07-03）；离散偏置误差棒待 TC.3 | TC.1 done；仍需 skyrmion 匹配分辨率标定（TC.3）定误差棒后才算完整"已确认" |
| 2 | 平动 G 是否≈0（闭环抵消） | 已确认（收敛已 PASS，非"未确认"） | TC.1 done；相对噪声底报，非字面 0 |
...
1. **T0.1** 合并 20260612 分支（Claude，git）→ 解锁一切负结果引用。✅ 完成（master @ 612e220，GATE G0 PASS）。
2. **TC.1** G/D 收敛阶梯（论文枢纽）→ 决定 C 线成败。✅ **完成，GATE GC1 PASS**（2026-07-03，`results_thiele_GD_convergence_20260703/`，bd Hopfion-960）。
3. **TC.4** 实测偏转角 → C 线预言的靶子。**下一步**（TC.1 PASS 后，GC-FINAL 唯一剩余前置项）。
