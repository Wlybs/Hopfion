...
鉴于三维周期性边界条件（PBC）具备空间平移不变性，可通过坐标平移修正上述初始偏心现象。具体方法是将弛豫收敛后的三维磁化矩阵沿 $z$ 轴进行循环移位（采用 NumPy 的 np.roll 函数），使霍普夫子质心重新对齐至计算域中心 $(\SI{25}{nm}, \SI{25}{nm}, \SI{25}{nm})$。由于该操作属于整体坐标变换，磁构型的内部自旋排列与拓扑属性未受影响。

平移操作本身可能引入微扰,居中后构型的稳定性因此需要独立检验。本文选取三组具有代表性的各向异性配置（$K_{u1} = 0$、$\SI{10e3}{}$、$\SI{50e3}{J/m^3}$）,在 $\alpha = 0.2$ 与 PBC 条件下各演化 \SI{1}{ns},期间持续记录 霍普夫子 质心坐标与 Hopf 指数 $Q_H$（经 mumax3 内建函数 ext\_hopfindex\_solidanglefourier 以 Berg-Lüscher 格点法配合涌现场的傅里叶变换计算得到，是霍普夫子拓扑构型存续的严格依据）。稳态判定标准设定为 $z$ 方向漂移量 $|\Delta z| < \SI{2}{nm}$。
