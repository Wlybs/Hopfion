...
Ku1_values = [1e2,5e2,1e3]
#Dbulk_values= [1e-6,5e-6,1e-5,5e-5,1.15e-4 , 5e-4, 1e-3, 5e-3, 1e-2, 5e-2]
# Bex 系数 beta 的候选值 (无单位, Bex = A * beta)
beta_values = [1e5,1.1e5,1.2e5,1.3e5,1.4e5,1.5e5,1.6e5,1.7e5,1.8e5,1.9e5,2e5] # 示例值，你可以按需修改
...
def run_single_simulation(ku1, beta):
    """生成一个mx3文件并运行Mumax3"""
    run_name = f"Ku1_{ku1:.1e}_Ms_{beta:.4e}"
...
def main():
    ...
    for ku1 in Ku1_values:
        for beta in beta_values:
            ...
            # --- 步骤 1: 运行仿真 ---
            sim_success, out_dir = run_single_simulation(ku1, beta)
