...
def readOvf(path):
    ...
    while True:
        ...
        if line.find(b'valuemultiplier') != -1:
            ...
            config['valuemultiplier'] = valuemultiplier = int(1) #mumax3-convert转换成text后，没有valuemultiplier了，所以手动补一个
        if line.find(b'valuedim') != -1:
            config['valuedim'] = valuedim = int(line[12:])
        else:
            config['valuedim'] = valuedim = int(3) #mumax3-convert转换成text后，没有valuedim了，所以手动补一个
        
        if line.find(b'begin: data') != -1:
            datatype = line[14:]
            # datatype = b'binary 8' #强制转换为

            # print(config)
            for key in ['znodes', 'ynodes', 'xnodes', 'valuedim']:
                assert key in config
            # ordered with x incremented first, then y, and finally z.
            data = readDataBlock(f, datatype, xnodes*ynodes*znodes).reshape(znodes, ynodes, xnodes, valuedim)
            #print(data)
            #print(data.shape)
            # data_block_begin_flag = 1
            data_block_begin_flag = 2 #强制改为2，表示已退出
            break
        if line.find(b'end: data') != -1:
...
def readDataBlock(f, datatype, count):
    ...
    if datatype == b'binary 4':
        ...
        # loadtxt自动可以把科学计数法转化为float
        data = np.loadtxt(f, dtype=float, skiprows=0, max_rows=0+count) #这里不用skip，已经从30行开始了
