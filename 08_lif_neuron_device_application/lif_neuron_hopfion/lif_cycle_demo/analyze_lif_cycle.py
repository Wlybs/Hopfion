...
from paper_style import setup_paper_style, save_paper_fig
...
# 相位英文 → 中文（图内标注用）
PHASE_CN = {
    "Pulse 1": "脉冲1", "Pulse 2": "脉冲2", "Pulse 3": "脉冲3", "Pulse 4": "脉冲4",
    "Leak 1": "漏1", "Leak 2": "漏2", "Leak 3": "漏3",
    "Fire+Reset": "发放+复位", "Refractory": "不应期",
}

# ── Paths ────────────────────────────────────────────────────────────────────
_BASE = "/mnt/d/Research/Hopfion/08_lif_neuron_device_application/lif_neuron_hopfion/lif_cycle_demo"
...
def _shade_phases(ax, phases=PHASES, ylim=None):
    ...
    for label, t0, t1, kind in phases:
        ...
        ax.text((t0 + t1) / 2, ylim[1] - 0.05 * (ylim[1] - ylim[0]),
                PHASE_CN.get(label, label), ha="center", va="top",
                fontsize=7, color="#333")


def plot_lif_cycle(t, z, title, output_path, phases=PHASES):
    fig, ax = plt.subplots(figsize=(10, 4.6))
    ax.plot(t, z, color="steelblue", lw=1.6, label=r"Hopfion $z$ 质心（膜电位）")
    ...
    ax.set_ylabel(r"Hopfion $z$ 质心 (nm)")
    ax.legend(loc="lower right")
    save_paper_fig(fig, output_path)
    plt.close(fig)
