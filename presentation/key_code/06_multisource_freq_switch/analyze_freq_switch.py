"""
频率切换双向z控制分析 (freq_switch_bidirectional)

单srcZ源，通过切换频率实现 +z/-z 反转：
  Phase 1 (0.00-0.25ns): srcZ @ 100GHz  → +z (anomalous mode)
  Phase 2 (0.25-0.50ns): srcZ @ 1100GHz → -z (normal mode)
  Phase 3 (0.50-0.60ns): OFF            → free relaxation

输出 results/:
  - freq_switch_z_control.png   (Fig 4-4: dz + vz 双面板)
  - freq_switch_z_control.txt   (数值汇总)
"""
import sys
import os
import numpy as np
...
OUT_DIR = os.path.join(HERE, "freq_switch_bidirectional_v2.out")
...
PHASE_BOUNDARIES = [0.10, 0.50, 0.60]
PHASE_LABELS = [
    "srcZ@100GHz (0→0.10ns)",
    "srcZ@1100GHz (0.10→0.50ns)",
    "OFF (0.50→0.60ns)",
]
...
def main():
    ...
    print(f"总位移: dz(0)={dz[0]:+.3f} → dz(end)={dz[-1]:+.3f} nm")
    ...
    print("\n=== 成功判据 ===")
    if len(phase_dz) >= 2:
        p1_up   = phase_dz[0]["delta"] > 0.1
        p2_down = phase_dz[1]["delta"] < -0.1
    ...
    # --- Text summary ---
    txt_path = os.path.join(RESULTS_DIR, "freq_switch_z_control.txt")
    with open(txt_path, "w") as f:
        f.write("=" * 70 + "\n")
        f.write("Freq-Switch Bidirectional Z Control Summary\n")
        f.write("srcZ@100GHz(+z) → srcZ@1100GHz(-z) → OFF, B=1T\n")
        f.write("=" * 70 + "\n\n")
    ...
    ax1.set_title('Freq-Switch Bidirectional Z Control: srcZ@100GHz(+z) → srcZ@1100GHz(-z)',
                  fontsize=12)
    ...
    plt.tight_layout()
    out_png = os.path.join(RESULTS_DIR, "freq_switch_z_control.png")
