...
def produce_derived(
    ...
    *,
    ...
    generated_at_ns = max(time.time_ns(), metadata.st_mtime_ns)  # DrvFs clock skew
...
def validate_derived_outputs(
    ...
    *,
    ...
    validate_derived_evidence_bindings(recipes, evidence_rows)
...
DERIVED_EVIDENCE_COLUMNS = tuple(DerivedEvidence.__dataclass_fields__)
_GENERATION_TOKEN_PATTERN = re.compile(r"[0-9a-f]{32}\Z")


def _parse_evidence_integer(raw: str, field_name: str) -> int:
    if not isinstance(raw, str) or not raw.isdecimal() or str(int(raw)) != raw:
        raise DerivedDataError(
            f"{field_name} must be a canonical nonnegative integer"
        )
    return int(raw)


def parse_derived_evidence_rows(
    rows: Sequence[Mapping[str, str]],
) -> tuple[DerivedEvidence, ...]:
    """Convert strict CSV-shaped mappings into typed immutable evidence rows."""

    expected = set(DERIVED_EVIDENCE_COLUMNS)
    parsed: list[DerivedEvidence] = []
    for number, row in enumerate(rows, start=2):
        if set(row) != expected or any(
            not isinstance(value, str) for value in row.values()
        ):
            raise DerivedDataError(
                f"derived evidence row {number} does not match the versioned schema"
            )
        if row["is_complete_field"] != "false":
            raise DerivedDataError(
                "derived evidence is_complete_field must be the literal false"
            )
        try:
            parent_figure_ids = _required_ids(
                row["parent_figure_ids"], "parent_figure_ids"
            )
            parent_data_ids = _required_ids(
                row["parent_data_ids"], "parent_data_ids"
            )
            parsed.append(
                DerivedEvidence(
                    ...
                    output_size=_parse_evidence_integer(
                        row["output_size"], "output_size"
                    ),
                    output_mtime_ns=_parse_evidence_integer(
                        row["output_mtime_ns"], "output_mtime_ns"
                    ),
                    parent_figure_ids=parent_figure_ids,
                    parent_data_ids=parent_data_ids,
                    environment_command=row["environment_command"],
                    executed_python=row["executed_python"],
                    is_complete_field=False,
                    generation_token=row["generation_token"],
                    generated_at_ns=_parse_evidence_integer(
                        row["generated_at_ns"], "generated_at_ns"
                    ),
                )
            )
        except DerivedDataError as error:
            raise DerivedDataError(
                f"derived evidence row {number}: {error}"
            ) from error
    return tuple(parsed)


def _validate_evidence_parent_ids(
    values: object,
    field_name: str,
) -> tuple[str, ...]:
    if not isinstance(values, tuple) or not values or not all(
        ...
        raise DerivedDataError(
            f"evidence {field_name} must be a non-empty immutable ID tuple"
        )
    return _required_ids(";".join(values), field_name)


def validate_derived_evidence_bindings(
    recipes: Sequence[DerivedRecipe],
    evidence_rows: Sequence[DerivedEvidence],
) -> None:
    """Bind immutable generation evidence to recipes without filesystem reads."""

    recipe_tuple = tuple(recipes)
    evidence_tuple = tuple(evidence_rows)
    recipe_by_id = {recipe.recipe_id: recipe for recipe in recipe_tuple}
    if len(recipe_by_id) != len(recipe_tuple):
        raise DerivedDataError("derived recipe IDs must be unique")
    for label, values in (
        ("output data IDs", tuple(recipe.output_data_id for recipe in recipe_tuple)),
        ("output paths", tuple(recipe.output_path for recipe in recipe_tuple)),
    ):
        if len(values) != len(set(values)):
            raise DerivedDataError(f"derived recipe {label} must be unique")

    evidence_by_id = {row.recipe_id: row for row in evidence_tuple}
    if len(evidence_by_id) != len(evidence_tuple):
        raise DerivedDataError("derived generation evidence IDs must be unique")
    for label, values in (
        ("output data IDs", tuple(row.output_data_id for row in evidence_tuple)),
        ("output paths", tuple(row.output_path for row in evidence_tuple)),
    ):
        if len(values) != len(set(values)):
            raise DerivedDataError(
                f"derived generation evidence {label} must be unique"
            )
    if set(evidence_by_id) != set(recipe_by_id):
        missing = sorted(set(recipe_by_id) - set(evidence_by_id))
        extra = sorted(set(evidence_by_id) - set(recipe_by_id))
    ...
    tokens = tuple(row.generation_token for row in evidence_tuple)
    if len(tokens) != len(set(tokens)):
        raise DerivedDataError("derived generation tokens must be unique")

    for recipe_id, recipe in recipe_by_id.items():
        evidence = evidence_by_id[recipe_id]
        if not _GENERATION_TOKEN_PATTERN.fullmatch(evidence.generation_token):
            raise DerivedDataError(
                f"{recipe_id}: generation token must be lowercase 128-bit hexadecimal"
            )
        for field_name, value in (
            ("output_size", evidence.output_size),
            ("output_mtime_ns", evidence.output_mtime_ns),
            ("generated_at_ns", evidence.generated_at_ns),
        ):
            if isinstance(value, bool) or not isinstance(value, int) or value <= 0:
                qualifier = (
                    "positive" if field_name == "output_size" else "a positive integer"
                )
                raise DerivedDataError(
                    f"{recipe_id}: {field_name} must be {qualifier}"
                )
        if evidence.generated_at_ns < evidence.output_mtime_ns:
            raise DerivedDataError(
                f"{recipe_id}: generated_at_ns must not precede output_mtime_ns"
            )
        try:
            require_relative_path(evidence.output_path)
            require_relative_path(evidence.source_path)
            require_relative_path(evidence.producer_script)
        except ManifestError as error:
            raise DerivedDataError(f"{recipe_id}: {error}") from error
        _require_sha256(evidence.source_sha256, "evidence source_sha256")
        _require_sha256(evidence.producer_sha256, "evidence producer_sha256")
        _require_sha256(evidence.output_sha256, "evidence output_sha256")
        parent_figure_ids = _validate_evidence_parent_ids(
            evidence.parent_figure_ids, "parent_figure_ids"
        )
        parent_data_ids = _validate_evidence_parent_ids(
            evidence.parent_data_ids, "parent_data_ids"
        )
        if evidence.is_complete_field is not False:
            raise DerivedDataError(
                f"{recipe_id}: evidence is_complete_field must be false"
            )
        expected_fields = {
            "output_data_id": recipe.output_data_id,
            "output_path": recipe.output_path,
            ...
            "parent_figure_ids": _required_ids(
                recipe.parent_figure_ids, "parent_figure_ids"
            ),
            "parent_data_ids": _required_ids(
                recipe.parent_data_ids, "parent_data_ids"
            ),
            "environment_command": recipe.environment_command,
