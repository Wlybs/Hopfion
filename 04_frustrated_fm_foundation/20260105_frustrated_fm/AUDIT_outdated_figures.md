# 审计：04_frustrated_fm_foundation/20260105_frustrated_fm/* 与毕设 figures/ 的不一致清单

> 生成时间：2026-05-14
> 范围：`04_frustrated_fm_foundation/20260105_frustrated_fm/` 全部子目录共 49 张 PNG
> 对照源：`09_paper_thesis_talks/bishe/thesis_v2/figures/`（毕设最终图，~80 张）+ `figure-mapping.md`（最后更新 2026-04-01）
> 用户决定：**只列清单不动手**
> 关键发现：毕设已用 Hopf 指数 $Q_H$ 替代 core-voxel 计数作为稳定性判据，但源仿真目录里仍残留 core-voxel 老图。`09_paper_thesis_talks/bishe/thesis_v2/figures/fig3-9_centered_core_count.png` 文件名仍为 core_count，caption 已改为 "$Q_H$ 随时间演化"（filename 没同步改）。
...
| # | 文件路径（相对 04_frustrated_fm_foundation/20260105_frustrated_fm/） | size | 论文引用 | 处理 |
|---|---|---|---|---|
...
**结果**：
- 5 张保留图（B01/B06/B07/B11/B12）已用 `95_shared_scripts/paper_style.py` 统一论文风格重画，输出到对应 `results/` 子目录
...
## F 类：毕设 `09_paper_thesis_talks/bishe/thesis_v2/figures/` 内部的文件名 ↔ caption / 编号不一致 ✓ 已处理 2026-05-14
...
来源：扫描 `09_paper_thesis_talks/bishe/thesis_v2/chapters/ch0*.tex` 所有 `\includegraphics` + `\caption` 配对得到。
...
🐛 `panel_label` 默认 `top_center_outside` 与单 axes 上的 `legend_above` (位置 (0.5, 1.02)) 重叠
   ...
   - 位置: `/mnt/d/Research/Hopfion/95_shared_scripts/paper_style.py`
...
- 毕设 figures: `/mnt/d/Research/Hopfion/09_paper_thesis_talks/bishe/thesis_v2/figures/`
- 章节 tex: `/mnt/d/Research/Hopfion/09_paper_thesis_talks/bishe/thesis_v2/chapters/ch04-stability.tex` (含 fig3-* 引用)
