...
def main():
    ...
    for (ku1, data), color in zip(all_data.items(), colors):
        if data is None:
            continue
        # 只画存活帧：fit_failed 会返回垃圾 R(~19nm)、r=None，须按 status 过滤
        ts_R = [(d[0], d[1]) for d in data if d[1] is not None and d[3] == "alive"]
        ts_r = [(d[0], d[2]) for d in data if d[2] is not None and d[3] == "alive"]
        label = fmt_ku1(ku1)
        if ts_R:
