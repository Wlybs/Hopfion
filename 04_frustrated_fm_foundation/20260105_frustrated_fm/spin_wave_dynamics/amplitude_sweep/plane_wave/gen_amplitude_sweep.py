"""
amplitude_sweep — .mx3 批量生成脚本

固定: f = 440 GHz, srcX_vibX
扫描: B 振幅 = 0.05, 0.1, 0.2, 0.5, 1.0, 2.0 T (共 6 个点)
运行: 0.5 ns / 点
初始态: centered Ku10k 平衡态 (m000020.ovf)

使用前修改 INIT_OVF 为本机路径。
"""
...
EnableDemag = false
MaxErr = 1e-4  // Relaxed for sweep (default 1e-5)
...
def amp_to_str(b):
    """0.05 -> '0p05', 1.0 -> '1p0'"""
    return f"{b:.2f}".rstrip("0").rstrip(".").replace(".", "p") or "0"


def main():
    print(f"Generating {len(AMPS)} .mx3 scripts for amplitude sweep ...")
    for b in AMPS:
        label = amp_to_str(b)
        fname = f"sw_B{label}T.mx3"
        fpath = os.path.join(HERE, fname)
        with open(fpath, "w") as fp:
            fp.write(TEMPLATE.format(b_amp=b, init_ovf=INIT_OVF))
        print(f"  {fname}  (B = {b} T)")
    print("Done.")
