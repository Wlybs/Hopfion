"""
freq_sweep — 运动模式分析（05ns 数据集）
对 300/400/900/1000GHz 4个频率点用相位相关法提取位移轨迹，绘制:
  1. r(t) 位移轨迹（分量 + 总位移）
  2. v(t) = dr/dt 速度（向量微分）
  3. a(t) = dv/dt 加速度
  4. 运动模式分类汇总表
...
def main():
    print("=" * 60)
    print("  freq_sweep: Motion Mode Analysis 05ns (phase correlation)")
    print("=" * 60)

    all_data = {}
    for freq in FREQS_GHZ:
        out_dir = os.path.join(HERE, "05ns", f"sw_f{freq}GHz.out")
    ...
    colors = plt.cm.tab10(np.linspace(0, 1, len(freq_profiles)))
    ...
    plt.suptitle("Frequency Sweep 05ns: Hopfion Displacement r(t)", fontsize=13)
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, "displacement_05ns.png"), dpi=200)
    plt.close()
    print(f"\n[fig] results/displacement_05ns.png")
    ...
    plt.suptitle("Velocity Profile 05ns: v(t) = dr/dt", fontsize=13)
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, "velocity_05ns.png"), dpi=200)
    plt.close()
    print(f"[fig] results/velocity_05ns.png")
    ...
    axes[2].set_ylabel("Mean |a| (nm/ns^2)")
    ...
    plt.suptitle("Motion Mode Frequency Map (05ns)", fontsize=13)
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, "motion_mode_map_05ns.png"), dpi=200)
    plt.close()
    print(f"[fig] results/motion_mode_map_05ns.png")

    # --- Summary table ---
    lines = [
        "freq(GHz) | |dr|(nm) |  dz(nm)  | v_mean(nm/ns) | a_mean(nm/ns^2) | Core_0 | Core_f | Mode",
        "-" * 100
    ]
    for freq in freqs:
        ...
        lines.append(
            ...
            f"| {np.mean(p['speed'][n3_idx:]):13.2f} | {np.mean(p['accel'][n3_idx:]):15.2f} "
            f"| {p['cores'][0]:6d} | {p['cores'][-1]:6d} | {p['mode']}"
        )

    txt = "\n".join(lines)
    with open(os.path.join(RESULTS_DIR, "motion_mode_summary_05ns.txt"), "w") as f:
        f.write(txt + "\n")
    print(f"\n[report] results/motion_mode_summary_05ns.txt")
    print("\n" + txt)
