"""
freq_sweep_extended_05ns — 关键频率时间延伸 0.5ns
本地: 300 + 900 GHz (匀速模式, 计算量小)
"""
import os
...
TEMPLATE = """\
// === Freq Sweep Extended 0.5ns: f = {freq_ghz} GHz, srcX_vibX ===
...
autosave(m, 1e-11)    // 10ps -> 50 frames over 0.5ns
tableautosave(1e-12)
TableAdd(E_Total)
...
for f in FREQS_GHZ:
    ...
    with open(os.path.join(HERE, fname), "w") as fp:
